...
| **Model Knowledge** | `[🧠]` | อธิบาย concept ทั่วไป, ยกตัวอย่างเปรียบเทียบ, ให้บริบท, Deep Dive | Architecture claims — ต้อง `[✓]` จาก repo |
| **Web Search** | `[🌐]` | เสริมเมื่อ repo docs ไม่พอ, benchmarks, community discussion | Architecture claims — ถ้าหาไม่เจอ → `[!]` |

### หลักการ

```
Architecture → [✓] จาก repo เท่านั้น
Concept/Context → [🧠] ใช้ความรู้โมเดลได้
ข้อมูลเสริม → [🌐] ค้นเน็ตได้
```
...
GGML คือ zero-dependency ML library [🧠] — แนวคิดเดียวกับ BLAS
Q4_K_M inference speed เร็วกว่า FP16 ถึง 3x [🌐] จาก llama.cpp benchmarks

❌ ผิด — architecture claim ใช้ [🧠]:
Llama.cpp ใช้ SQLite สำหรับ checkpoint persistence [🧠]
...
## Evidence Strength (5 เครื่องหมาย)
...
| `[🧠]` | Model Knowledge | LLM | concept ทั่วไป, บริบท, เปรียบเทียบ |
| `[🌐]` | Web Search | Internet | ข้อมูลเสริม, benchmarks, community |
...
□ capstone-project.md   # โปรเจคจบเล่ม (optional)
```
